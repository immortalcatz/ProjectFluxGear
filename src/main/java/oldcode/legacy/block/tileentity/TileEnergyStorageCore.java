...
import oldcode.legacy.util.helpers.MultiblockHelper.TileLocation;
import oldcode.legacy.util.block.tile.TileObjectSync;
...
public class TileEnergyStorageCore extends TileObjectSync {
	...
	private boolean areStabilizersActive() {
		for(int i = 0; i < this.stabilizers.length; ++i) {
			...
			if(!tile.stabalizerMode || this.field_145850_b.getBlockMetadata(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) != 1) {
				return false;
			}
		}
