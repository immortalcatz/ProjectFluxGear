package oldcode.projectfluxgear.to_refactor;
...
import oldcode.projectfluxgear.core.util.dynreg.OreEntries;
...
import oldcode.projectfluxgear.tech.item.tool.ItemPaintbrush;
import oldcode.projectfluxgear.tech.item.tool.ItemPrototypeSonicWrench;
import oldcode.projectfluxgear.util.helper.LoadedHelper;
import oldcode.projectfluxgear.util.item.tool.BucketFluxGear;
import oldcode.projectfluxgear.to_refactor.util.legacy.ContentRegistry;
import oldcode.projectfluxgear.to_refactor.util.legacy.block.BlockBase;
import oldcode.projectfluxgear.to_refactor.util.legacy.fluid.BlockFluidClassicWS;
import oldcode.projectfluxgear.to_refactor.util.legacy.fluid.BlockFluidReactive;
import oldcode.projectfluxgear.to_refactor.block.tile.TileTimeyWimey;
import oldcode.projectfluxgear.to_refactor.util.chemistry.reaction.ReactionSpec;
...
import oldcode.projectfluxgear.to_refactor.block.BlockPFGBasic.*;
import oldcode.projectfluxgear.core.util.dynreg.ManualBlockEntries.*;
import oldcode.projectfluxgear.core.common.config.FluxGearConfig;
import oldcode.projectfluxgear.to_refactor.fluid.BlockFluidAcid;
import oldcode.projectfluxgear.to_refactor.fluid.BlockFluidSmog;
import oldcode.projectfluxgear.core.item.ItemInteractivePFG;
...
public class FluxGearContent_ implements IFuelHandler{
	...
	public void initTimeyWimey() {
		...
		if (!FluxGearConfig.recursionTorches) {
			TileTimeyWimey.blacklistBlock(FluxGearContent_.timeyWimeyTorch);
    ...
    public void registerDispenserHandlers() {
        BlockDispenser.dispenseBehaviorRegistry.putObject(FluxGearContent_.itemBucket, new DispenserFilledBucketHandler());
    ...
    public static final void RegisterContent (ContentRegistry cr) {
        ...
        if (woodPlanks != null && woodPlanks.size() > 0) {
            for (ItemStack item : woodPlanks) {
                GameRegistry.addSmelting(item, FluxGearContent_.dustAshes, 0.0F);
            }
        }
    }
}