	...
	public void tickEnd() {
		...
		if(orphanedParts.size() > 0) {
			...
			for(Set<MultiblockControllerBase> mergePool : mergePools) {
				...
				for(MultiblockControllerBase controller : mergePool) {
					...
					FluxGearCore.log.fatal("Multiblock system checked a merge pool of size %d, found no master candidates. This should never happen.", mergePool.size());
				}
				else {
		...
		if(deadControllers.size() > 0) {
			for(MultiblockControllerBase controller : deadControllers) {
				...
				if(!controller.isEmpty()) {
					FluxGearCore.log.fatal("Found a non-empty controller. Forcing it to shed its blocks and die. This should never happen!");
