package mortvana.projectfluxgear.core.network;
...
public class FluxGearPacketHandler_ <T> {
	...
	public void setClientHandler(SimpleChannelInboundHandler<T> handler) {
		...
		if (handlers.get(Side.CLIENT) != null ) {
			channel.pipeline().remove("PFGClientHandler");
		}
		channel.pipeline().addAfter(codecName, "PFGClientHandler", handler);
	...
	public void setServerHandler(SimpleChannelInboundHandler<T> handler) {
		...
		if (handlers.get(Side.SERVER) != null) {
			channel.pipeline().remove("PFGServerHandler");
		}
		channel.pipeline().addAfter(codecName, "PFGServerHandler", handler);
