package mortvana.legacy.dependent.seconddegree.paintedstone.recipe;
...
public class RecipePaintbrush implements IRecipe {
	...
	public boolean matches(InventoryCrafting inventoryCrafting, World world) {
		...
		List required = new ArrayList(input);
		...
		for (int i = 0; i < inventoryCrafting.getSizeInventory(); ++i) {
			...
			if (tempStack != null) {
				...
				if (tempStack.getItem() == paintbrush.getItem()) {
					...
					foundBrush = true;
				} else if (tempStack.getItem() == dye.getItem() && tempStack.getMetadata() == dye.getMetadata() && countDye < 1702) {
					countDye++;
				} else {
					return false;
				}
