package mortvana.legacy.dependent.firstdegree.core.util.repack.multiblock.rectangular;

import net.minecraft.tileentity.TileEntity;
import net.minecraft.world.World;

import mortvana.legacy.clean.core.util.coordinates.CoordTriplet;
import mortvana.legacy.clean.core.util.repack.multiblock.MultiblockControllerBase;
...
public abstract class RectangularMultiblockControllerBase extends MultiblockControllerBase {
	...
	protected void isMachineWhole() throws MultiblockValidationException {
		...
		Class<? extends RectangularMultiblockControllerBase> clazz = this.getClass();
			...
			for(int y = minimumCoord.y; y <= maximumCoord.y; y++) {
				for(int z = minimumCoord.z; z <= maximumCoord.z; z++) {
					...
					if(te instanceof RectangularMultiblockTileEntityBase) {
						...
						if(!clazz.equals(part.getMultiblockControllerType())) {
							throw new MultiblockValidationException(String.format("Part @ %d, %d, %d is incompatible with machines of type %s", x, y, z, clazz.getSimpleName()));
						}
					} else {
