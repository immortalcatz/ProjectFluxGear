package mortvana.legacy.errored.core.util.repack.mutliblock;

import java.util.HashSet;
import java.util.LinkedList;
import java.util.Set;

import mortvana.legacy.clean.core.util.repack.multiblock.IMultiblockPart;
import mortvana.legacy.clean.core.util.repack.multiblock.MultiblockValidationException;
...
import mortvana.legacy.clean.core.util.coordinates.CoordTriplet;
...
public abstract class MultiblockControllerBase {
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if(!connectedParts.add(part)) {
			MeltedDashboardLib.log.warn("[%s] Controller %s is double-adding part %d @ %s. This is unusual. If you encounter odd behavior, please tear down the machine and rebuild it.", (worldObj.isRemote ? "CLIENT" : "SERVER"), hashCode(), part.hashCode(), coord);
		}
	...
	public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
		...
		if(!connectedParts.remove(part)) {
			MeltedDashboardLib.log.warn("[%s] Double-removing part (%d) @ %d, %d, %d, this is unexpected and may cause problems. If you encounter anomalies, please tear down the reactor and rebuild it.", worldObj.isRemote?"CLIENT":"SERVER", part.hashCode(), part.xCoord, part.yCoord, part.zCoord);
		}
	...
	public final void updateMultiblockEntity() {
		...
		// Not assembled - don't run game logic
			...
			if(minimumCoord != null && maximumCoord != null && this.worldObj.checkChunksExist(minimumCoord.x, minimumCoord.y, minimumCoord.z, maximumCoord.x, maximumCoord.y, maximumCoord.z)) {
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if(otherController == this) { return false; }
		...
		if(res < 0) { return true; }
			...
			MeltedDashboardLib.log.warn("[%s] Encountered two controllers with the same reference coordinate. Auditing connected parts and retrying.", worldObj.isRemote?"CLIENT":"SERVER");
			auditParts();
			...
			if(res < 0) { return true; }
				...
				MeltedDashboardLib.log.error("My Controller (%d): size (%d), parts: %s", hashCode(), connectedParts.size(), getPartsListString());
				MeltedDashboardLib.log.error("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(), otherController.connectedParts.size(), otherController.getPartsListString());
	...
	private void auditParts() {
		...
		MeltedDashboardLib.log.warn("[%s] Controller found %d dead parts during an audit, %d parts remain attached", worldObj.isRemote ? "CLIENT" : "SERVER", deadParts.size(), connectedParts.size());
	}
