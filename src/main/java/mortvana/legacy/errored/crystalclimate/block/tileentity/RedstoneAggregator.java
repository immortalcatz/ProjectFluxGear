package mortvana.legacy.errored.crystalclimate.block.tileentity;
...
public class RedstoneAggregator extends TileEntity {
	...
	void growCrystal() {
		if (crystalValue < 10) {
			...
			if (validBlock(block)) {
				...
				CrystalLogic crystal = (CrystalLogic) worldObj.getTileEntity(xCoord, yCoord + 1, zCoord);
	...
	boolean validBlock(Block block) {
		return block == CrystalClimate.crystalBlock;
	}
