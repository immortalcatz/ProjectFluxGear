...
public class TileEnergyStorageCore extends TileObjectSync {
	...
	private boolean findStablizers() {
		...
		for(z = this.field_145851_c; z <= this.field_145851_c + 11; ++z) {
			if(worldObj.getBlock(z, this.field_145848_d, this.field_145849_e) == ModBlocks.particleGenerator) {
				if(worldObj.getBlockMetadata(z, this.field_145848_d, this.field_145849_e) == 1) {
					flag = false;
				} else {
		...
		for(z = this.field_145849_e; z <= this.field_145849_e + 11; ++z) {
			if(worldObj.getBlock(this.field_145851_c, this.field_145848_d, z) == ModBlocks.particleGenerator) {
				if(worldObj.getBlockMetadata(this.field_145851_c, this.field_145848_d, z) == 1) {
					flag = false;
				} else {
	...
	private boolean testForOrActivateRedstone(int x, int y, int z, boolean set, boolean activate) {
		if(!activate) {
			if(set) {
				...
				return worldObj.getBlock(x, y, z) == Blocks.redstone_block || worldObj.getBlock(x, y, z) == ModBlocks.invisibleMultiblock && worldObj.getBlockMetadata(x, y, z) == 1;
			}
		} else {
	...
	private void activateStabilizers() {
		for(int capacity = 0; capacity < this.stabilizers.length; ++capacity) {
			...
			worldObj.setBlockMetadataWithNotify(this.stabilizers[capacity].getXCoord(), this.stabilizers[capacity].getYCoord(), this.stabilizers[capacity].getZCoord(), 1, 2);
		}
	...
	public void deactivateStabilizers() {
		for(int i = 0; i < this.stabilizers.length; ++i) {
			if(this.stabilizers[i] == null) {
				...
				TileParticleGenerator tile = worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) != null && worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) instanceof TileParticleGenerator?(TileParticleGenerator)worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()):null;
				if(tile != null) {
					tile.stabilizerMode = false;
					worldObj.setBlockMetadataWithNotify(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord(), 0, 2);
				}
			}
		}
	...
	private boolean areStabilizersActive() {
		for(int i = 0; i < this.stabilizers.length; ++i) {
			...
			if(!tile.stabilizerMode || worldObj.getBlockMetadata(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) != 1) {
				return false;
			}
		}
	...
	private boolean checkStabilizers() {
		...
		while(i < this.stabilizers.length) {
			...
			TileParticleGenerator gen = worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) != null && worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()) instanceof TileParticleGenerator?(TileParticleGenerator)worldObj.getTileEntity(this.stabilizers[i].getXCoord(), this.stabilizers[i].getYCoord(), this.stabilizers[i].getZCoord()):null;
