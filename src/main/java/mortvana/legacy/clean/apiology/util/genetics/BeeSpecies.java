...
public enum BeeSpecies implements IAlleleBeeSpecies, IIconProvider {
	...
	public BeeSpecies addProduct(ItemStack produce, int percentChance) {
		products.put(produce, .01F * percentChance);
		return this;
	}
    ...
    public BeeSpecies addSpecialty(ItemStack produce, int percentChance) {
        specialties.put(produce, .01F * percentChance);
        return this;
    }
    ...
    public void registerIcons(IIconRegister iconRegister) {
	    ...
	    String iconPath = "forestry:bees/default/";

        IIcon body1 = iconRegister.registerIcon(iconPath + "body1");
        ...
        for (int i = 0; i < EnumBeeType.values().length; i++) {
            ...
            icons[i][0] = iconRegister.registerIcon(iconPath + EnumBeeType.values()[i].toString().toLowerCase(Locale.US) + ".outline");
            icons[i][1] = (EnumBeeType.values()[i] != EnumBeeType.LARVAE) ? body1 : iconRegister.registerIcon(iconPath + EnumBeeType.values()[i].toString().toLowerCase(Locale.US) + ".body");
            icons[i][2] = iconRegister.registerIcon(iconPath + EnumBeeType.values()[i].toString().toLowerCase(Locale.US) + ".body2");
        }
    }
