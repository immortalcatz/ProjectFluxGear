...
public abstract class MultiblockControllerBase {
	...
	public void detachBlock(MultiblockPart part, boolean chunkUnloading) {
		...
		if(!connectedParts.remove(part)) {
			MeltedDashboardCore.logger.warn("[%s] Double-removing part (%d) @ %d, %d, %d, this is unexpected and may cause problems. If you encounter anomalies, please tear down the reactor and rebuild it.", worldObj.isRemote?"CLIENT":"SERVER", part.hashCode(), part.xCoord, part.yCoord, part.zCoord);
		}
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if(res < 0) { return true; }
			...
			MeltedDashboardCore.logger.warn("[%s] Encountered two controllers with the same reference coordinate. Auditing connected parts and retrying.", worldObj.isRemote?"CLIENT":"SERVER");
			auditParts();
			...
			if(res < 0) { return true; }
				...
				MeltedDashboardCore.logger.error("My Controller (%d): size (%d), parts: %s", hashCode(), connectedParts.size(), getPartsListString());
				MeltedDashboardCore.logger.error("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(), otherController.connectedParts.size(), otherController.getPartsListString());
	...
	private void auditParts() {
		...
		MeltedDashboardCore.logger.warn("[%s] Controller found %d dead parts during an audit, %d parts remain attached", worldObj.isRemote ? "CLIENT" : "SERVER", deadParts.size(), connectedParts.size());
	}
